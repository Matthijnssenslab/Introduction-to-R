---
title: "R introduction November 2023"
author: "Lab of Viral Metagenomics KU Leuven"
output: 
  html_document:
    df_print: paged
...
```{r eval=FALSE}
#NB:Packages are installed once but loaded every session
#YOU CLICK "Packages", "Install" and type the name of the package
...
First, we will make a dataframe of `iris` for each species separately. This will help us in the rest of the course to keep things a bit more clean.
```{r}
setosa <- iris %>% 
  filter(Species=="setosa")
head(setosa)
...
#### Check for equal variance (F-test)
...
#### Student's t-test to compare means between 2 groups
...
We tested the normality of `Sepal.Width` for all species [here](#Check for normal distribution (Shapiro-Wilk test)) and found that the sepal width of every species was normally distributed.
...
**Exercise:** Perform a linear regresion analyis on the R `cars` dataset. This dataset contains data on the stopping distance of cars that are travelling at different speeds.
```{r}
#Solution:
